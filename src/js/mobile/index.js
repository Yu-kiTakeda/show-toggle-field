 ...
 */
  const events = [
    ...
    ...options.reduce((condFields, opt) => {
      ...
      return events.concat(['mobile.app.record.create.change.', 'mobile.app.record.edit.change.'].map(e => e + field));
    }, [])
  ];
  kintone.events.on(events, function(event) {
    ...
    const targetOptions = eTypeLastWord === 'show' ? 
      options.filter(opt => (!opt.applyPage || opt.applyPage === 'all') ? true : (opt.applyPage === 'detail' && event.type === 'mobile.app.record.detail.show') ? true : (opt.applyPage === 'edit' && (event.type === 'mobile.app.record.edit.show' || event.type === 'mobile.app.record.create.show')) ? true : false)
